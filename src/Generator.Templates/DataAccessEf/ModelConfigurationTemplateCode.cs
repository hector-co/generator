...
namespace Generator.Templates.DataAccessEf
{
    public partial class ModelConfigurationTemplate
    {
        ...
        {
            return modelDefinition.EvalProperties.Values.Where(p => p.IsSystemType && p.IsGeneric)
        ...
        {
            return modelDefinition.EvalProperties.Values.Where(p => p.IsEntityType && !p.IsGeneric)
                .Select(p => p.Name).Distinct().ToList();
        }

        private static List<PropertyDefinition> GetManyToOneProperties(ModelDefinition modelDefinition)
        {
            return modelDefinition.EvalProperties.Values.Where(p => p.IsEntityType && p.IsCollection && !p.WithMany)
                .Distinct().ToList();
        }
    }
}
